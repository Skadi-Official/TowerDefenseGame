...
public:
	CoinProp()
	{
		...
		velocity.x = (rand() % 2 ? 1 : -1) * 2 * SIZE_TILE;			//一个向右一个向左
...
private:
	Vector2 position;
	Vector2 velocity;			//金币掉落的速度
	Timer timer_jump;			//金币掉落的动画的持续时间
	Timer timer_disappear;		//在地图上多久后消失
