...
public:
	Enemy()
	{
		...
		timer_skill.set_on_timeout([&]() { on_skill_released(this); });//计时器触发时调用
	...
	void on_update(double delta)
	{
		...
		// 怪物的移动
		Vector2 move_distance = velocity * delta;//一帧移动的距离
		Vector2 target_distance = position_target - position;//前一帧离目标距离
	...
	void set_position(const Vector2& position)//生成敌人的位置
	{
	...
	double get_route_process() const
	{
		...
		return (double)idx_target / (route->get_idx_list().size() - 1);//行进了多少距离（百分比）
	}
protected:
	Vector2 size;
...
private:
	...
	SkillCallback on_skill_released;//释放技能时调用

	Timer timer_restore_speed;//速度恢复的时间

	const Route* route = nullptr;//在哪条路径上移动
	int idx_target = 0;//目标所在单元格
	Vector2 position_target;//目标位置，世界坐标下的位置，计算用
