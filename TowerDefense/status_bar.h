...
public:
	...
	void on_render(SDL_Renderer* renderer)
	{
		...
		static const ResourcesManager::TexturePool& tex_pool =
		...
		roundedBoxRGBA(renderer, rect_dst.x, rect_dst.y,
			rect_dst.x + rect_dst.w, rect_dst.y + rect_dst.h, 2,
...
private:
	...
	const int width_mp_bar = 200;										//冷却条的宽度
	const int height_mp_bar = 20;										//冷却条的高度
	const int width_border_mp_bar = 4;									//冷却条边框的宽度
	const SDL_Point offset_shadow = { 2,2 };							//实现阴影效果所需要的偏移
...
private:
	...
	int width_text = 0, height_text = 0;									//绘制硬币数量所需要的矩形的参数
